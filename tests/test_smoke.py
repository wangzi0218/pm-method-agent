...
class OrchestratorSmokeTest(unittest.TestCase):
    ...
    def test_http_service_can_serve_web_demo_shell_and_assets(self) -> None:
        ...
        html_body = html_response.encoded_body().decode("utf-8")
        self.assertIn("seedWorkspaceButton", html_body)
        self.assertIn("workspaceMemory", html_body)
        self.assertIn("项目背景、当前焦点和你的偏好会收在这里", html_body)
        self.assertIn("门店核销漏执行", html_body)
        self.assertIn("首帖率偏低", html_body)
        self.assertIn("新人优惠浮层", html_body)
        self.assertIn("门店店员最近反馈 H5 上核销后还要回头查订单", html_body)
        self.assertIn("新用户注册后 3 天内发帖率偏低", html_body)
        self.assertIn("想在商品详情页加一个新人优惠浮层", html_body)
        ...
        self.assertIn('"mini-program": "小程序"', js_body)
    ...
    def test_http_service_can_list_workspace_cases_and_switch_active_case(self) -> None:
        ...
        self.assertIn("cases", switch_response.payload)
        self.assertEqual(switch_response.payload["cases"]["active_case_id"], first_case_id)
        self.assertIn("workspace_memory", switch_response.payload["cases"])
    ...
    def test_runtime_session_query_start_exposes_recovery_summary(self) -> None:
        ...
        self.assertEqual(payload["resume_point"]["kind"], "approval")
        self.assertEqual(payload["resume_point"]["suggested_action"], "resolve-approval")
