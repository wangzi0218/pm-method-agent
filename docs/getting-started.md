...
如果你只是想先快速看一眼效果，不想自己手工编几轮输入，网页里现在也可以直接点：

- `装载示例`

这一步会优先尝试用当前配置的模型服务生成一组中文示例案例；如果模型不可用，会自动回退到内置样本，不会让 demo 卡住。
...
其中 `case_runtime` 用来承接这一轮的增强摘要和模型回退状态，不需要自己再去拆 `case.metadata`。
