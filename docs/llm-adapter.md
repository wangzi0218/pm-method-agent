...
现在案例相关响应会把模型参与情况收敛成统一的 `understanding` 字段，CLI 和网页都消费这份结果。

它主要区分几类状态：

- `local-only`：未启用模型，本轮由本地规则和方法运行时完成。
- `llm-assisted`：模型参与了语义理解或文案增强，但阶段推进和关口仍由 runtime 控制。
- `partial-fallback`：部分模型增强生效，部分增强失败后回到本地规则。
- `llm-fallback`：模型不可用，已回到本地判断。
- `contract-fallback`：模型返回内容不满足当前契约，已回到本地判断。
- `llm-configured`：模型已配置，但当前轮没有触发模型增强组件。
...
3. 继续细化 `understanding` 的组件级解释，让用户知道是哪一层增强或回退。
