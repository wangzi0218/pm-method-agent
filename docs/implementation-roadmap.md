...
- 已完成 `v0.2.0` 发布，入口形态、连续追问和记忆闭环已经具备最小预览版
- 已补 `v0.2` 产品规则清单，用来冻结当前主线规则、暂定规则和未定边界
- 已补“阶段结论判定清单”，开始把“什么时候继续问、什么时候先给结论”单独收口
- 已补“记忆写入与防线设计”，把自然记忆、显式确认、可见引用和可撤回边界先定清楚
- 已补连续追问回答承接、阶段转场提示和网页可见的“这轮怎么承接”
- 已补记忆建议、用户确认、记忆管理、记忆引用展示和“不是这个项目”动作
- 当前进入 `v0.3` 方向收敛：优先评估 IDE / Skill 入口，并同步收敛 MCP 外壳边界
- 当前尚未补正式 IDE skill 包、MCP 外壳实现、sub-agent 编排和更完整的外部集成示例
...
- 如果目标是继续验证 `PM Method Agent` 作为方法内核，`v0.3` 可以按下面路径继续。
- 如果目标是面向产品经理个人或团队提效做一个更简洁的新产品，建议把当前仓库收尾为参考实现，另开新项目。
- 新项目启动建议见 [项目收尾说明](project-closeout.md) 和 [新产品启动 Brief](new-product-brief.md)。

`v0.3` 如果继续推进，更适合继续：

1. 先按 [v0.3 方向设计](v0-3-direction.md) 收敛 IDE / Skill 入口。
2. 更新 [IDE / skill 最小契约](ide-skill-minimal-contract.md)，明确触发、输入、输出、承接和记忆边界。
3. 补一个本地 skill 使用草稿，复用现有 CLI / HTTP 服务，不先做复杂 UI。
4. 按 [MCP 候选工具清单](mcp-tool-candidates.md) 收敛 tools 分级，但先不替代 HTTP 服务。
5. 网页演示只做必要的小幅同步，不升级成完整后台。
...
- 已补 `understanding` 理解方式契约，让 CLI、HTTP 和网页能统一展示本地规则、模型辅助和回退状态
- 已补组件级理解解释，开始归一化 LLM 失败原因，并明确这些增强失败默认不影响主线阶段推进
- 已补共享 LLM 契约解析层，让回复理解、前置收敛、文案增强、追问润色和示例生成共用 JSON object 校验与空结果回退语义
...
- 已补最小 `query_loop` 查询循环摘要，让 CLI、HTTP 和网页能直接展示单轮处理路径
- 已补最小 `resume_suggestions`，把待审批、阻塞、暂缓、失败和活跃案例转成可展示的继续建议
- 已补最小 `runtime/resume-actions` 继续动作入口，把建议动作分发到消息、审批或运行时查看
- 已补最小 `resume_point`，把底层恢复点翻译成用户可理解的恢复语义
- 已补最小 `runtime_contract`，把运行状态、循环状态、终止语义、事件类型和继续动作收成稳定枚举
